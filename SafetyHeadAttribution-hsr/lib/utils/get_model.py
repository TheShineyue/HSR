...
from lib.utils.custommodel import (
        CustomLlavaNextForConditionalGenerationLlama,
    )
...
def get_model(model_name, accelerator=None, add_size=False, get_custom=False):
    if accelerator is not None:
        ...
        if not get_custom:
            ...
            model = CustomLlavaNextForConditionalGenerationLlama.from_pretrained(model_name)
        else:
            raise ValueError("")
        if add_size:
            model.resize_token_embeddings(model.config.vocab_size + 1)
